...
namespace Project2
{
    class Utility
    {
            ...
            {
                var  hasNumber = new Regex("(\\d+)");  // Number
                var hasLowercase = new Regex("([a-z])"); // Any lowercase letter
                var hasUppercase = new Regex("([A-Z])"); // Any uppercase letter
                var hasNoWhitespace = new Regex(@"([^\s -])"); //No whitespace
                var hasSpecialChar = new Regex("([!.@#$%^&*()])"); //Special character
                var hasMinimum8Chars = new Regex(@".{8,20}");

            if (hasNumber.IsMatch(password) && hasLowercase.IsMatch(password) && hasUppercase.IsMatch(password) && hasNoWhitespace.IsMatch(password) && hasSpecialChar.IsMatch(password) && hasMinimum8Chars.IsMatch(password)) return true; else return false;
